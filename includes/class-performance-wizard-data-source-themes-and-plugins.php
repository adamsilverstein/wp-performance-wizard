...
class Performance_Wizard_Data_Source_Themes_And_Plugins extends Performance_Wizard_Data_Source_Base {
	...
	public function __construct() {
		...
		$this->set_description( 'The Themes and Plugins data source describes the active theme and the active plugins on this site, including each plugin\'s metadata and, where available, its public quality signals from the WordPress.org plugin and theme directories. When the administrator has opted in, it may also include selected plugin source files.' );
		$this->set_analysis_strategy(
			'Correlate this data with the Lighthouse and HTML findings: for each failing audit or problematic asset already identified, use the asset path (assets enqueued by an extension typically live under /wp-content/plugins/{slug}/... or /wp-content/themes/{slug}/...) to attribute the issue to a specific plugin or theme listed here, and then make a concrete recommendation that names that extension.
		When source_files are present for a plugin, you may cite specific code patterns in those files as evidence for a finding; otherwise base conclusions only on the metadata and the correlated Lighthouse/HTML evidence, and do not speculate about a plugin\'s internals.
		Use the WordPress.org quality signals - overall rating, the distribution of 1- through 5-star reviews, and support_threads versus support_threads_resolved - as supporting context when comparing alternatives or recommending that a plugin be replaced or removed, but treat low ratings as a signal, not proof of a performance problem. Only discuss plugins and the theme that actually appear in this data.'
		);
